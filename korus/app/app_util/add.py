...
def add_deployment(conn, logger):
    ...
    owner = ui.UserInput("owner", "Data owner/collector", group="deployment").request(
        logger
    )
    name = ui.UserInput(
        "name", "Descriptive name for the deployment", group="deployment"
    ).request(logger)
    start_utc = ui.UserInput(
        "start_utc", "UTC start date or time (e.g. 2020-06-01)", group="deployment"
    ).request(logger)
    end_utc = ui.UserInput(
        "end_utc", "UTC end date or time (e.g. 2020-06-01)", group="deployment"
    ).request(logger)
    location = ui.UserInput(
        "location",
        "Geographic location (e.g. Salish Sea, BC, Canada)",
        group="deployment",
    ).request(logger)
    latitude_deg = ui.UserInput(
        "latitude_deg",
        "Latitude (degrees)",
        transform_fcn=float,
        group="deployment",
    ...
    sample_rate = ui.UserInput(
        "sample_rate",
        "Sampling rate in samples/s per channel",
        transform_fcn=int,
        group="deployment",
    ).request(logger)
    data_license = ui.UserInput(
        "data_license",
        "License/terms for access to and use of data",
        group="deployment",
        unknown=True,
    ).request(logger)
    hydrophone = ui.UserInput(
        "hydrophone", "Hydrophone make and model", group="deployment"
    ).request(logger)
    bits_per_sample = ui.UserInput(
        "bits_per_sample", "Bits per sample", transform_fcn=int, group="deployment"
    ).request(logger)
    num_channels = ui.UserInput(
        "num_channels",
        "Number of recording channels",
        transform_fcn=int,
        group="deployment",
    ).request(logger)
    sensitivity = ui.UserInput(
        "sensitivity",
        "Hydrophone sensitivitiy in dB re 1 V/muPa",
        transform_fcn=float,
        group="deployment",
        unknown=True,
    ).request(logger)
    comments = ui.UserInput(
        "comments", "Any additional observations", group="deployment", unknown=True
    ).request(logger)
...
def add_job(conn, logger):
    ...
    annotator = ui.UserInput(
        "annotator", "Annotator (initials only)", group="job"
    ).request(logger)
    start_utc = ui.UserInput(
        "start_utc", "UTC start date (e.g. 2020-06-01)", group="job"
    ).request(logger)
    end_utc = ui.UserInput(
        "end_utc", "UTC end date (e.g. 2020-06-01)", group="job"
    ).request(logger)
    is_exhaustive = ui.UserInputYesNo(
        "is_exhaustive", "Were all primary sounds annotated? [y/N]", group="job"
    ).request(logger)
    ...
    while True:
        if ui_add_another_issue.request():
            description = ui.UserInput(
                "add_issue", "Briefly describe the issue"
            ).request(logger)
            issues.append(description)

            ui_add_another_issue.message = (
                "Do the annotations have any other known issues? [y/N]"
            )
        else:
            break
...
def create_timestamp_parser(group=None, logger=None):
    ...
    ui_ts_siz.add_option(
        key=["s", "smru"], message="SMRU timestamp format", fcn=lambda x: 19
    )
...
def add_files(conn, deployment_id, start_utc, end_utc, logger):
    ...
    def transform_fcn(x):
        ...
        if id not in ids:
            raise ValueError(
                f"Database does not contain data storage location with ID {id}"
            )
    ...
    (audio_path,) = (
        conn.cursor()
        .execute(f"SELECT path FROM storage WHERE id = {storage_id}")
        .fetchall()[0]
    )
    ...
    cprint(
        f" ## Scanning for {audio_format} files ... (this may take a while)", "yellow"
    )
...
def add_annotations(conn, deployment_id, job_id, logger, timestamp_parser=None):
    ...
    }  # filename -> (file ID, deployment ID) mapping

    # load taxonomy
    c = conn.cursor()
    tax_id = c.execute(f"SELECT taxonomy_id FROM job WHERE id = '{job_id}'").fetchall()[
        0
    ][0]
    ...
    while True:
        try:
            ...
            df["job_id"] = job_id
            df["file_id"] = df.path.apply(
                lambda x: file_id_map.get(os.path.basename(x), [0])[0]
            )
            if len(deployment_id) == 1:
                df["deployment_id"] = deployment_id[0]
            else:
                df["deployment_id"] = df.path.apply(
                    lambda x: file_id_map[os.path.basename(x)][1]
                )
            ...
            if "tag" in df.columns.values:
                ...
                if len(new_tags) > 0:
                    cprint(
                        f"\n ## The selection table contains the following new tags: {new_tags}",
                        "red",
                    )
            ...
            start = datetime.now()
            annot_ids = kdb.add_annotations(
                conn, annot_tbl=df, job_id=job_id, progress_bar=True
            )
            end = datetime.now()
    ...
    cprint(
        f"\n ## Successfully added {len(annot_ids)} annotations to the database in {(end - start).total_seconds():.2f} seconds",
        "yellow",
    )
    ...
    if num_missing > 0:
        cprint(
            f"\n ## WARNING: {num_missing} of the annotations pertain to audio files not present in the database",
            "red",
        )
    ...
    if len(annot_ids) > 0:
        print_summary = ui.UserInputYesNo(
            "print_summary", "Print summary? [y/N]", group="annotation"
        ).request()
...
def unique_from_list(x):
    ...
    value_count = dict(
        sorted(value_count.items(), key=lambda item: item[1], reverse=True)
    )
    return value_count
...
def from_raven(
    ...
    cprint(
        f"\n ## Found {len(input_paths)} selection tables matching the search criteria",
        "yellow",
    )
    ...
    if "Granularity" in df_in.columns.values:
        df_out["granularity"] = df_in["Granularity"].apply(
            lambda x: x.lower() if isinstance(x, str) else granularity
        )
    ...
    df_out = validate_annotations(
        df_out, tax, interactive=interactive, progress_bar=progress_bar
    )
    ...
    if timestamp_parser is not None:
        ...
        def get_start_utc(row):
            ...
            return timestamp_parser(row.path) + timedelta(
    ...
    df_out.sound_source = df_out.sound_source.apply(
        lambda x: x if x != "" else tax.get_node(tax.root).tag
    )

    # replace empty tentative sound source with None
    df_out.tentative_sound_source = df_out.tentative_sound_source.apply(
    ...
    df_out.tentative_sound_type = df_out.tentative_sound_type.apply(
        lambda x: x if x != "" else None
    )
...
def print_annotation_summary(conn, indices=None):
    ...
    counts = df.value_counts(
        ["tentative_sound_source", "tentative_sound_type"], dropna=True
    )
    counts = pd.DataFrame({"Count": counts})
...
def validate_annotations(df, tax, interactive=True, progress_bar=False):
    ...
    def _parse_ambiguous(labels, tax, label_map, idx, row, note=""):
        ...
        for label in labels:
            if interactive:
                label, label_map = validate_label_interactive(
                    label, tax, label_map, idx, row, note
                )
            else:
        ...
        while True:
            ...
            try:
                ...
                if row["ambiguous_sound_source"] is None:
                    s = row["sound_source"]
                    if "/" in s:
                        sources, src_map = _parse_ambiguous(
                            s.split("/"), tax, src_map, idx, row
                        )
                ...
                if row["ambiguous_sound_type"] is None:
                    ...
                    t = row["sound_type"]
                    if "/" in t:
                        types, typ_map = _parse_ambiguous(
                            t.split("/"), typ_tax, typ_map, idx, row
                        )
                ...
                ss = row["sound_source"]
                if interactive:
                    row["sound_source"], src_map = validate_label_interactive(
                        ss, tax, src_map, idx, row
                    )
                else:
                ...
                if interactive:
                    row["tentative_sound_source"], src_map = validate_label_interactive(
                        tss, tax, src_map, idx, row
                    )
                else:
                ...
                ambiguous_sources, src_map = _parse_ambiguous(
                    ass, tax, src_map, idx, row
                )
                ...
                if interactive:
                    row["sound_type"], typ_map = validate_label_interactive(
                        st, typ_tree, typ_map, idx, row, note
                    )
                else:
                ...
                if interactive:
                    row["tentative_sound_type"], typ_map = validate_label_interactive(
                        tst, typ_tree, typ_map, idx, row, note
                    )
                else:
                ...
                types, typ_map = _parse_ambiguous(
                    ast, typ_tree, typ_map, idx, row, note=note
                )
                row["ambiguous_sound_type"] = types
                ...
                if interactive:
                    ...
                    warn_msg = (
                        f"{str(e)}. Entry {idx} flagged as containing invalid data."
                    )
                    cprint(f"\n ## WARNING: {warn_msg}", "red")
