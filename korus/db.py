...
def filter_annotation(
    ...
    if source_type is not None:
        # @source_type
        if not invert:
            wc = _select_label_condition(
                conn, source_type, strict, tentative, ambiguous, taxonomy_id
            )
            where_conditions.append(wc)

            if exclude is not None:
                wc = _exclude_label_condition(
                    conn, exclude, strict, tentative, ambiguous, taxonomy_id
                )
                where_conditions.append(wc)

        # @source_type with @invert=True
        else:
            wc = _invert_select_label_condition(
                conn, source_type, strict, tentative, ambiguous, taxonomy_id
            )
            where_conditions.append(wc)
    ...
    if tag is not None:
        rows = c.execute(
            f"SELECT id FROM tag WHERE name IN {list_to_str(tag)}"
        ).fetchall()
    ...
    if tag is None or ktb.AUTO_NEG not in tag:
        (auto_neg_id,) = c.execute(
            f"SELECT id FROM tag WHERE name = '{ktb.AUTO_NEG}'"
        ).fetchall()[0]
    ...
    if granularity is not None:
        rows = c.execute(
            f"SELECT id FROM granularity WHERE name IN {list_to_str(granularity)}"
        ).fetchall()
...
def _select_label_condition(
    ...
    ids = get_label_id(
        conn, source_type=source_type, taxonomy_id=taxonomy_id, always_list=True
    )

    # crosswalk to all taxonomies, including descendants
    ids_crosswalk = dict()
    for tax_id in tax_ids:
        ids_x = klb.crosswalk_label_ids(
            conn, ids, dst_taxonomy_id=tax_id, ascend=False, descend=True, equiv=True
        )
...
def _exclude_label_condition(
    ...
    for tax_id in tax_ids:
        ids_x = klb.crosswalk_label_ids(
            conn, ids, dst_taxonomy_id=tax_id, ascend=True, descend=True, equiv=True
        )
        ids_crosswalk[tax_id] = np.unique(ids_x).tolist()
    ...
    for tax_id in tax_ids:
        ids_x = klb.crosswalk_label_ids(
            conn, ids, dst_taxonomy_id=tax_id, ascend=True, descend=False
        )
...
def _invert_select_label_condition(
    conn, source_type, strict, tentative, ambiguous, taxonomy_id
):
    """Helper function for @filter_annotation.
    ...
    for tax_id in tax_ids:
        ids_x = klb.crosswalk_label_ids(
            conn, ids, dst_taxonomy_id=tax_id, ascend=True, descend=True
        )
        ids_crosswalk[tax_id] = np.unique(ids_x).tolist()
...
def _primary_sound_condition(conn, source_type, taxonomy_id):
    ...
    for job_id, tax_id, prim_sound in rows:
        ...
        num_matches = 0
        for sound in sounds:
            ancestor_sounds = klb.get_related_label_id(
                conn, sound, ascend=True, descend=False, always_list=True
            )
...
def assign_files_to_job(conn, job_id, file_id, channel=0, extendable=True):
    ...
    if not extendable:
        query = (
            f"SELECT EXISTS(SELECT 1 FROM file_job_relation WHERE job_id = {job_id})"
        )
        assert not c.execute(query).fetchone()[
            0
    ...
    for fid, chan_list in zip(file_id, channel):
        for chan in chan_list:
            v = {"job_id": job_id, "file_id": fid, "channel": chan}
            try:
                ...
                logging.error(
                    f"Failed to insert {v} into file_job_relation table. Ignoring entry."
                )
                continue
...
def add_annotations(conn, annot_tbl, job_id, progress_bar=False, error="replace"):
    ...
    if (
        "sound_source" not in annot_tbl.columns
        and "excluded_sound_source" in annot_tbl.columns
    ):
    ...
    required_cols = ["file_id", "sound_source", "sound_type"]
    for col_name in required_cols:
        assert (
            col_name in annot_tbl.columns
        ), f"{col_name} missing from annotation table"
    ...
    if "duration_ms" not in annot_tbl.columns:
        ...
        def _duration_ms_fcn(filename, start_ms):
            return int(
                file_tbl.loc[filename].num_samples
                / file_tbl.loc[filename].sample_rate
                * 1e3
                - start_ms
            )

        annot_tbl["duration_ms"] = annot_tbl.apply(
            lambda r: _duration_ms_fcn(r.filename, r.start_ms), axis=1
        )
    ...
    def insert_annotation(c, file_tbl, row):
        ...
        if "deployment_id" in row and file_data is not None:
            assert (
                row.deployment_id == file_data["deployment_id"]
            ), "deployment IDs do not match"
        ...
        if row.tentative_sound_source is None and row.tentative_sound_type is None:
            ...
            if (
                row.tentative_sound_source is None
                and row.tentative_sound_type is not None
            ):
                tent_ss = row.sound_source
                tent_st = row.tentative_sound_type
            elif (
                row.tentative_sound_source is not None
                and row.tentative_sound_type is None
            ):
        ...
        v = {
            ...
            "start_utc": (
                start_utc.strftime("%Y-%m-%d %H:%M:%S.%f")[:-3]
        ...
        if "tag" in row:
            ...
            rows = c.execute(
                f"SELECT id FROM tag WHERE name IN {list_to_str(tags)}"
            ).fetchall()
        ...
        if (
            "freq_min_hz" in row
            and row.freq_min_hz is not None
            and not np.isnan(row.freq_min_hz)
        ):
        ...
        if (
            "freq_max_hz" in row
            and row.freq_max_hz is not None
            and not np.isnan(row.freq_max_hz)
        ):
        ...
        if "granularity" in row:
            rows = c.execute(
                f"SELECT id FROM granularity WHERE name = '{row.granularity}'"
            ).fetchall()
            if len(rows) == 0:
                raise ValueError(
                    f"Unrecognized annotation granularity: {row.granularity}"
                )
        ...
        if error_handling == REPLACE and (
            ...
            new_fmin = 0
            new_fmax = (
                256000 if file_data is None else int(file_data["sample_rate"] // 2)
            )
            ...
            warn_msg = (
                "Invalid frequency range replaced with default range to allow insertion into database:"
    ...
    for idx, row in tqdm(
        annot_tbl.iterrows(), disable=not progress_bar, total=annot_tbl.shape[0]
    ):
        while True:
...
def add_negatives(conn, job_id):
    ...
    assert (
        is_exhaustive
    ), "not possible to automatically generate 'negatives' for non-exhaustive annotation jobs"
    ...
    annot_tbl["deployment_id"] = annot_tbl.file_id.apply(
        lambda x: file_tbl.loc[x].deployment_id
    )
...
def get_annotations(conn, indices=None, format="korus", label=None):
    ...
    assert (
        isinstance(format, str) and format.lower() in valid_formats
    ), f"Invalid table format. The valid formats are: {valid_formats}"
    ...
    cols = [
        (
            ...
            "korus_id",
            "int",
        ),  # (table ref, input column name, output column name, dtype)
        ("a", "job_id", "job_id", "int"),
    ...
    annot_tbl["tag"] = None
    annot_tbl.loc[~idx, "tag"] = annot_tbl.loc[~idx].tag_id.apply(
        lambda x: [tag_map[tag_id] for tag_id in x]
    )
    ...
    annot_tbl.loc[~idx, "ambiguous_label"] = annot_tbl.loc[
        ~idx
    ].ambiguous_label_id.apply(lambda x: lookup_label(x))
...
def _convert_to_ketos(annot_tbl, label, conn):
    ...
    for idx, row in df.iterrows():
        ...
        for j, file_id in enumerate(row.file_id_list):
            ...
            file_duration_ms = 1000.0 * num_samples / sample_rate
            new_row = row.copy()
            if j > 0:
                new_row.start_ms = 0

            new_row.duration_ms = min(remaining_ms, file_duration_ms - new_row.start_ms)
            new_row.start_utc = row.start_utc + timedelta(
                microseconds=1e3 * (row.duration_ms - remaining_ms)
            )
            data.append(new_row)
    ...
    # rename
    annot_tbl.rename(
        columns={"freq_min_hz": "freq_min", "freq_max_hz": "freq_max"}, inplace=True
    )
...
def _convert_to_raven(annot_tbl, conn):
    ...
    df_out["Delta Time (s)"] = annot_tbl.duration_ms / 1000.0
    df_out["File Offset (s)"] = annot_tbl.start_ms / 1000.0
    df_out["Begin Time (s)"] = annot_tbl["offset"] + df_out["File Offset (s)"]
    df_out["End Time (s)"] = (
        annot_tbl["offset"] + df_out["File Offset (s)"] + df_out["Delta Time (s)"]
    )
    df_out["Begin Path"] = annot_tbl.apply(
        lambda r: os.path.join(str(r.top_path), str(r.relative_path), r.filename),
        axis=1,
    )
...
def find_negatives(file_tbl, annot_tbl, max_gap_ms=100, tag_id=1):
    ...
    def annot_start_utc_fcn(r):
        return file_tbl.loc[r.filename].start_utc + timedelta(
            microseconds=r.start_ms * 1e3
        )

    annot_tbl["start_utc"] = annot_tbl.apply(lambda r: annot_start_utc_fcn(r), axis=1)
    annot_tbl["end_utc"] = annot_tbl.apply(
            ...
            for chan in [int(c) for c in file_row.channel.split(";")]:
                ...
                if chan in negative:
                    ...
                    if chan in end_utc_prev:
                        gap_ms = (
                            file_start_utc - end_utc_prev[chan]
                        ).total_seconds() * 1000
                ...
                annot_tbl_fil = annot_tbl_chan[
                    (annot_tbl_chan.index >= file_start_utc)
                    & (annot_tbl_chan.index < file_row.end_utc)
                ]

                # loop over selected annotations
                for start_utc, annot_row in annot_tbl_fil.iterrows():
                    if start_utc <= negative[chan]["start_utc"]:
                        negative[chan]["start_utc"] = max(
                            annot_row.end_utc, negative[chan]["start_utc"]
                        )
                    else:
    ...
    neg_tbl["start_ms"] = neg_tbl.apply(
        lambda r: (r.start_utc - r.file_start_utc).total_seconds() * 1000, axis=1
    ).astype(int)
    neg_tbl["duration_ms"] = neg_tbl.apply(
        lambda r: (r.end_utc - r.start_utc).total_seconds() * 1000, axis=1
    ).astype(int)
...
def build_file_table(conn, job_id, top=False):
    ...
    channels = (
        pd.DataFrame(rows, columns=["file_id", "channel"])
    ...
    # add end_utc column
    file_tbl["end_utc"] = file_tbl.apply(
        lambda r: r.start_utc
        + timedelta(microseconds=r.num_samples / r.sample_rate * 1e6),
        axis=1,
    )
    ...
    file_tbl["channel"] = file_tbl.file_id.apply(
        lambda x: join(channels.loc[x].channel)
    )

    # sort according to deployment and time, in that order
    file_tbl.sort_values(
        by=["deployment_id", "start_utc", "channel", "end_utc"], inplace=True
    )
...
def insert_row(conn, table_name, values):
    ...
    c.execute(
        f"INSERT INTO {table_name} ({col_names}) VALUES ({val_str})",
        list(values.values()),
    )
...
def insert_job(conn, values):
    ...
    if tax_id is not None:
        if "primary_sound" in values:
            try:
                values["primary_sound"] = json.dumps(
                    get_label_id(
                        conn, values["primary_sound"], tax_id, always_list=True
                    )
                )
                ...
                err_msg = (
                    f"Attempt to convert primary sound tags {v} to label IDs failed\n"
        ...
        if "background_sound" in values:
            try:
                values["background_sound"] = json.dumps(
                    get_label_id(
                        conn, values["background_sound"], tax_id, always_list=True
                    )
                )
                ...
                err_msg = (
                    f"Attempt to convert background sound tags {v} to label IDs failed\n"
                    + traceback.format_exc()
                )
                raise RuntimeError(err_msg)
