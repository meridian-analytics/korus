...
def get_related_label_id(
    conn, label_id, ascend=False, descend=False, always_list=False
):
...
def crosswalk_label_id(
    conn, label_id, dst_taxonomy_id, ascend=False, descend=False, always_list=False
):
    ...
    if org == dst:
        ...
        mapped_label_id += get_related_label_id(
            conn, mapped_label_id, descend=descend, ascend=ascend, always_list=True
        )
    ...
    # trace node history
    mapped_source_id, source_is_equivalent = trace_node_history(
        conn, source_id, dst, mode
    )
