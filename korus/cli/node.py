...
def create_nodes(db: Database) -> dict[str, Node]:
    ...
    # table selection (main/root node)
    nodes[main_id] = Node(id=main_id, name="main", fcn=prompt.select_table, args=(db,))

    # table action selection
    for table_name in db.tables.keys():
        id = node_id(table_name)
        nodes[id] = Node(
            id=id, name=table_name, fcn=prompt.select_table_action, args=(table_name,)
        )
    ...
    for table_name in db.tables.keys():
        ...
        nodes[id] = Node(id=id, name="info", fcn=vw.view_info, args=(db, table_name))
        ...
        nodes[id] = Node(id=id, name="add", fcn=add.add, args=(db, table_name))

        # update
        id = node_id(table_name, "update")
        nodes[id] = Node(id=id, name="update", fcn=upd.update, args=(db, table_name))
