...
def test_get_label_id(basic_db):
    ...
    l = kdb.get_label_id(
        conn, ("SRKW", "S01"), taxonomy_id=tax_id, ascend=False, descend=False
    )
    assert df.loc[l].ss == "SRKW"
    assert df.loc[l].st == "S01"

    l_list = kdb.get_label_id(
        conn, ("SRKW", "%"), taxonomy_id=tax_id, ascend=False, descend=False
    )
    for l in l_list:
        assert df.loc[l].ss == "SRKW"
    ...
    l_list = kdb.get_label_id(
        conn, ("SRKW", "S01"), taxonomy_id=tax_id, ascend=True, descend=False
    )
    for l in l_list:
    ...
    l_list = kdb.get_label_id(
        conn, ("KW", "PC"), taxonomy_id=tax_id, ascend=False, descend=True
    )
    for l in l_list:
    ...
    with pytest.raises(ValueError):
        kdb.get_label_id(
            conn, ("Fish", "PC"), taxonomy_id=tax_id, ascend=False, descend=True
        )

    with pytest.raises(ValueError):
        kdb.get_label_id(
            conn, ("Fish", "%"), taxonomy_id=tax_id, ascend=False, descend=True
        )

    with pytest.raises(ValueError):
        kdb.get_label_id(
            conn, ("%", "LoudSound"), taxonomy_id=tax_id, ascend=False, descend=True
        )
