...
def test_crosswalk_label(basic_db):
    ...
    label_org = ("KW", "PC")
    label_dst, is_equiv = klb.crosswalk_label(
        conn, label_org, origin_taxonomy_id=2, dst_taxonomy_id=1, always_list=True
    )
    ...
    label_org = ("SRKW", "S01")
    label_dst, is_equiv = klb.crosswalk_label(
        conn, label_org, origin_taxonomy_id=2, dst_taxonomy_id=1
    )
    assert label_dst == [("KW", "PC")]
    ...
    label_org = ("NRKW", "PC")
    label_dst, is_equiv = klb.crosswalk_label(
        conn, label_org, origin_taxonomy_id=2, dst_taxonomy_id=3
    )
    assert label_dst == [("KW", "PC")]
    assert is_equiv == False
    ...
    label_org = ("KW", "PC")
    label_dst, is_equiv = klb.crosswalk_label(
        conn, label_org, origin_taxonomy_id=1, dst_taxonomy_id=4
    )
